...
2025年の最新ベストプラクティスに基づき、AIの推論精度および出力量の安定性を向上させるため、以下の最適化を適用しています。
...
**手動作業に関する注意事項:**
- 推論レベル (`reasoning_effort`) の変更により、GitHub ModelsのAPI制限（Rate limits）により早く到達する可能性があります。特に `PAT_FOR_MODELS` を使用して高負荷な推論を利用するワークフロー（AI Test Generator など）を実行する際は、利用枠やクオータの消費状況に注意してください。
