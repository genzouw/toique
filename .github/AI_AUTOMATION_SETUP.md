...
### Gemini CLI GitHub Actions (PR Review & Issue Triage)

2024年に公開されたGoogleの `run-gemini-cli` を利用し、`Gemini CLI`ベースのAIプルリクエストレビューとIssue Triageを導入しています。

**設定手順:**

1. 個人のGoogleアカウントを使用して[Google AI Studio](https://aistudio.google.com/)からGemini APIキーを取得します。(無料枠があります)
2. GitHub リポジトリの **Settings → Secrets and variables → Actions** を開きます。
3. **New repository secret** をクリックし、名前を `GEMINI_API_KEY` とし、取得した API キーを値として保存します。
4. このリポジトリでは `.gemini/commands/` 配下のファイルはカスタムプロンプトとしてGit管理されていますが、ローカルで利用する際の認証情報などが漏洩しないよう `.gitignore` で `.gemini/*` を無視するように設定（ただし `.gemini/commands/` は例外）しています。
