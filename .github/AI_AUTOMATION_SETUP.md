...
> **適用範囲についての注記:** 第3節で撤去した CI 上の AI ワークフロー（リポジトリ内で AI 推論を直接実行するもの）は
> 本規約の当初の対象でした。現在は `gemini-review.yml` / `gemini-issue-triage.yml`
> （第5節・「Gemini API による PR レビューおよび Issue トリアージの導入」節参照）が本規約の対象です。
> これらのワークフローは `derailed-dash/gemini-review-action` を通じて、PR の差分・リポジトリコンテキスト・
> 過去のレビューコメント・Issue のタイトルと本文を Gemini に渡すため、以下のタグ付けルールを適用してください。
