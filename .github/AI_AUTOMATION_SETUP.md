...
**注意:** Dependabot が作成する Pull Request は通常の Actions シークレットにアクセスできないため、この機能を利用するには **Settings → Secrets and variables → Dependabot** を開き、以下の2つのシークレットを Dependabot 専用のシークレットとして手動で追加する必要があります。
