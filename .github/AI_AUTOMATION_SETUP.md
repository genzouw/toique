...
**ただし、無料枠（Google AI Studio 経由など）が提供されている Gemini に限り、コードレビューおよび issue トリアージの精度向上を目的とした例外として、リポジトリ側ワークフロー（`gemini-review.yml`）での採用を認めます。**
...
- **外部AIプロバイダの API キーを要するもの（Gemini を除く）**: リポジトリ側でAPIキーを管理するワークフローは、原則として新規追加しません。

**例外的に採用するもの:**

- **Gemini**: 無料枠（Google AI Studio 経由など）が提供されているため、コードレビューおよび issue トリアージの精度向上を目的として、`gemini-review.yml` に限定して採用しています（現行方針の唯一の例外。上記の「リポジトリ側で AI 推論を実行するワークフローは新規に追加しない」原則に対する明示的な例外です）。
...
1. **Gemini API Key の設定**: `gemini-review.yml` による AI コードレビューと Issue トリアージを利用するため、Google AI Studio 等で発行した `GEMINI_API_KEY` をリポジトリの **Actions Secrets** に登録してください。このキーは `gemini-2.5-flash` モデルの呼び出しに利用されます。
