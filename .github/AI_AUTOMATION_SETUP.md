...
> **適用範囲についての注記:** `ai-pr-review.yml` により CI 上で Gemini を用いた AI レビューを再導入しています。
> ただし同ワークフローのプロンプト構築は本リポジトリのコードではなく、サードパーティ製 Action
> ([petarzarkov/gemini-code-review-action](https://github.com/petarzarkov/gemini-code-review-action)) の内部実装が担っており、
> 本リポジトリ側で本規約（タグによる外部由来コンテキストの分離）を直接適用することはできません。
> このリスクへの対応状況は #739 で追跡しています。本リポジトリ自身がプロンプトを組み立てる AI ワークフローを
> 実装する場合（第5節の方針上、原則として行いません）は、本規約を必ず適用してください。
