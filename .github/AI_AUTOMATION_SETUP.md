...
AI によるレビュー・トリアージは、リポジトリ側に API キーも課金設定も必要としない外部 App（CodeRabbit / Qodo Merge、第4節参照）に一本化します。ただし上記の Gemini AI Code Review と AI Accessibility Scanner は、無料枠の Gemini API を利用する明示的な例外として本リポジトリで採用しています。
