...
### Gemini Code Review の導入

生成AIを用いた高度なレビュー自動化トレンドに合わせ、`derailed-dash/gemini-review-action` を利用したAI PR Code Review ワークフロー (`.github/workflows/gemini-review.yml`) を新設しました。

**手動作業に関する注意事項:**

- リポジトリの Secrets に `GEMINI_API_KEY` を設定してください。
- 実行権限に関するセキュリティ上の制約から、このワークフローはフォークからのPRでは動作しません。内部PRでのみ動作します。
- このワークフローは、除外対象を除く追跡済みのテキストファイルのコード内容をコードベースコンテキストとして外部の Google Gemini サービスへ送信します。機密情報・個人情報・proprietary code が含まれる場合は外部サービスへ送信され得るため、事前にデータ分類を行い、機密ファイルを除外する方針を定めた上で、Google Gemini のデータ保存・利用ポリシーを確認してください。
- **信頼前提**: 本ワークフローは `pull_request` イベントで動作するため、対象となる内部PR（同一リポジトリ内のブランチ）の作成者は、GitHub の既定の仕様上すでにリポジトリへの書き込み権限と Secrets へのアクセス権を持ちます。したがって `GEMINI_API_KEY` および書き込み可能な `GITHUB_TOKEN` の利用は、内部PR作成者を信頼済みとする前提の運用です。フォーク外部からの実行を許可しない現状では、レビュー処理と特権処理（コメント投稿等）をジョブ分離する追加対応は行いません。外部コラボレーターを受け入れる場合は、本節を見直した上でジョブ分離を再検討してください。
...
### Web検索基盤の最適化 (`ai-web-search/action.yml`)
