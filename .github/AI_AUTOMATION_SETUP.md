...
上記の方針に基づき、当リポジトリのAI機能を活用するためには、以下のAPIキーを GitHub のリポジトリシークレットとして登録する必要があります。
プルリクエストをマージする前に、以下の手順を必ず実施してください。

1. **必要なAPIキーの取得**:
   - Google AI Studio にて **Gemini API Key** を取得してください（無料枠あり）。
   - Tavily API にて **Tavily API Key** を取得してください（無料枠あり）。
   - Exa API にて **Exa API Key** を取得してください（無料枠あり）。
2. **GitHub Secrets への登録**:
   - GitHubリポジトリの **Settings** を開く。
   - 左側メニューの **Secrets and variables** > **Actions** を選択する。
   - 以下の名前で **New repository secret** を作成し、取得したキーをそれぞれ登録する。
     - `GEMINI_API_KEY`
     - `TAVILY_API_KEY`
     - `EXA_API_KEY`
3. 登録が完了すると、AIによるコードレビューやアクセシビリティスキャンなどのワークフローが正常に動作するようになります。
