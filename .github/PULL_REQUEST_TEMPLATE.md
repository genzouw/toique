...
なし（AI自動化に関連するPRの場合は、下記「AI関連の前提条件確認」を参照）

## AI関連の前提条件確認

<!-- 本PRがAI自動化に関連する場合、マージ前に以下の手動設定が完了していることを確認してください。関連しない場合はこのセクションごと削除して構いません。 -->

- [ ] `GEMINI_API_KEY` がリポジトリのシークレットとして設定されていること
- [ ] RAG検索機能で Exa または Tavily を使用する場合、対応する `EXA_API_KEY` または `TAVILY_API_KEY` が設定されていること（DuckDuckGoを使用する場合はAPIキー不要）
- [ ] `.github/workflows/**/*.yml` で追加・変更した外部ActionがすべてコミットSHAに固定されていること、および各ジョブの `permissions` が必要最小限のスコープになっていること
