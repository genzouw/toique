...
- [ ] 開発者はローカル環境で変更を pull し、`pre-commit install && pre-commit run --all-files` でフック環境をセットアップ・検証してください（`pre-commit autoupdate` はレビュー・SHA固定済みの `rev` を書き換えてしまうため使用しないでください）。
