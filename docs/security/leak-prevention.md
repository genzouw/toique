...
| `pr-detect-secrets-review.yml`           | 比較対象の `.secrets.baseline` を PR のベースブランチ側に固定しているため、PR 内でベースラインを更新しても検知は解除されない（`rev` 更新 PR が構造的に失敗する。握りつぶし防止とのトレードオフ） | `detect-secrets.yml`（HEAD のベースラインを使うため同一 PR 内で解消する）                                 |
