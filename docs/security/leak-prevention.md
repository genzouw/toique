...
2. **CI 検知 (CI/CD)**
   ...
   - **役割:** ローカルでの検知をすり抜けたシークレットや、CI環境特有の漏洩を検知します。`secretlint` によりファイルシステム全体の静的解析を行います。また、公式の `gitleaks-action` を導入しており、Pull Request 内でシークレットの混入を検知した場合には自動的にエラーコメント（アノテーション）が付与されます。ブランチ保護ルールで本ワークフローのパスを必須に設定することで、マージをブロックできます。これにより、コードレビュー段階で漏洩に気づきやすくなります。さらに TruffleHog が補完的に動作して、実効性の高い Verified Secrets（クラウドプロバイダ等に到達可能な本物の鍵）をスキャンします。また、`osv-scanner` を用いて依存関係における脆弱性走査を行い、悪意のあるパッケージを通じたサプライチェーン攻撃（例: 実行時に `.env` の内容を外部へ送信する等のシークレット漏洩）のリスクを低減します。さらに、`CodeQL` の `security-extended` クエリを有効化しており、変数名や関数の引数などソースコードの文脈を深く解析し、ハードコードされたシークレットや認証情報の漏洩 (CWE-798 など) を検知します。
