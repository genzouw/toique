...
```bash
cd backend
DATABASE_URL=postgresql://toique:toique@localhost:5433/toique PORT=3001 npm run dev
```

サーバが `http://localhost:3001` で起動します。
...
```bash
curl -X POST http://localhost:3001/api/v1/line-channels \
...
```bash
curl -s http://localhost:3001/api/v1/messages | jq
```
...
- **404 Not Found**: ポートを `3001` で起動しているか確認
