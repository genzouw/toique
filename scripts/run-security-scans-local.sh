...
if [ -n "$GITLEAKS_CMD" ]; then
  ...
  # --config は明示必須で、未指定だとカスタムルール（allowlist 等）が読み込まれず
  # リポジトリ全体で許可しているはずの誤検知が保存時だけ再発する。一方 .gitleaksignore は
  # 既定のファイル名であればカレントディレクトリから自動探索されるが（gitleaks 8.30.1 で
  # 確認済み）、「無いと壊れる必須指定」ではなく意図を明確にするための冗長指定として
  # --gitleaks-ignore-path も明示している（PR #864 の自己レビュー指摘）。
  #
