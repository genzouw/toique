...
describe('AdminLogin', () => {
  ...
  it('renders correctly', () => {
    ...
    expect(screen.getByLabelText(/ユーザーID/, { selector: 'input' })).toBeInTheDocument();
  ...
  it('shows error and removes auth token on unauthenticated path (login failure)', async () => {
    ...
    // Fill in credentials
    const usernameInput = screen.getByLabelText(/ユーザーID/, { selector: 'input' });
  ...
  it('navigates to admin dashboard on successful login', async () => {
    ...
    // Fill in credentials
    await user.type(screen.getByLabelText(/ユーザーID/, { selector: 'input' }), 'admin');
    await user.type(screen.getByLabelText(/パスワード/, { selector: 'input' }), 'correctpassword');
