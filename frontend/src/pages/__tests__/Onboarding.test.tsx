...
describe('Onboarding', () => {
  ...
  it('renders correctly', () => {
    ...
    expect(screen.getByRole('heading', { name: '組織の登録' })).toBeInTheDocument();
    expect(screen.getByLabelText('組織名')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: '組織を作成して開始' })).toBeInTheDocument();
  });
  ...
  it('shows error message when createTenant throws an error', async () => {
    ...
    const submitButton = screen.getByRole('button', { name: '組織を作成して開始' });
  ...
  it('navigates to dashboard on successful tenant creation', async () => {
    ...
    vi.mocked(api.createTenant).mockResolvedValueOnce({ id: '1', name: 'Test Tenant' });
